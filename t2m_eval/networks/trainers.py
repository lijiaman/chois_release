...
class DecompTrainerV3(object):
    ...
    def forward(self, batch_data):
        motions = batch_data['motion']
        self.motions = motions.detach().to(self.device).float()
        # self.latents = self.movement_enc(self.motions[..., :-4])
        self.latents = self.movement_enc(self.motions) # BS X 30(window_size//4) X 512
    ...
    def train(self, train_dataloader, val_dataloader, plot_eval, ds):
        ...
        while epoch < self.opt.max_epoch:
            ...
            if epoch % self.opt.eval_every_e == 0:
                ...
                data = torch.cat([self.recon_motions[:, None, :, :], \
                                self.motions[:, None, :, :]], dim=1) # BS X 2 X T X D 
                # import pdb 
                # pdb.set_trace() 
                save_dir = pjoin(self.opt.eval_dir, 'E%04d' % (epoch))
                os.makedirs(save_dir, exist_ok=True)
                plot_eval(data[:4], save_dir, ds)
...
class TextMotionMatchTrainer(object):
    ...
    def forward(self, batch_data):
        ...
        # word_emb = batch_data['word_embeddings']
        # pos_ohot = batch_data['pos_one_hots']
        # caption = batch_data['caption']
        # cap_lens = batch_data['caption_len']
        # motions = batch_data['motion']
        # m_lens = batch_data['seq_len']
        ...
        '''Movement Encoding'''
        # movements = self.movement_encoder(motions[..., :-4]).detach() # The original version seems to exclude feet contact label 
